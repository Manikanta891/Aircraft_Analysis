...
class ParkingEditor(SingleBoxEditor):
    def __init__(self, image_path: str):
        super().__init__("Parking Setup", image_path)
        self.label_prefix = "P"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Parking Setup')
    ...
    editor = ParkingEditor(img_path)

    if args.load and os.path.exists(args.load):
        with open(args.load, 'r') as f:
            data = json.load(f)
            if 'parking_boxes' in data:
                editor.boxes = [(b['x1'], b['y1'], b['x2'], b['y2']) for b in data['parking_boxes']]
            elif 'boxes' in data:
